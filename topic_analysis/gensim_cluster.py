...
class ReviewModel(object):
    ...
    def closest_words(self, word):
        """
        Method to find the closest words for a query using cosine similarity of
        the word vectors. Returns the words above the cutoff set for the model.
        INPUT:
            word: query word
        OUTPUT:
        ...
        if word not in self.topic:
            self.topic[word] = set([item[0] for item in
                                    self.model.most_similar(word, topn=100) if
                                    item[1] > self.similarity])
        return [word] + list(self.topic[word])

    def load_data(self, filename):
        """
        Loads the review data from json.
        INPUT:
